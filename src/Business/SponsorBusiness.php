...
readonly class SponsorBusiness
{
    ...
    {
        $sponsorIdsTotal = $this->sponsorRepository->findFilteredSponsorIdsPaginated($page, $limit, $sort, $order, $name, $contact, $status, $counterpartType, $minAmount, $maxAmount, $otherCounterpart, $hasContract);
        $sponsors = $this->sponsorRepository->findSponsorsByIds($sponsorIdsTotal['items']);

        $sponsorPersons = [];
        /** @var Sponsor $sponsor */
        foreach ($sponsors as $sponsor) {
            $sponsorArray = $this->serializer->normalize($sponsor, null, ['groups' => ['sponsor', 'sponsorLinks', 'sponsorPerson', 'person', 'personLinks', 'link', 'linkLinkType', 'linkType']]);

            $sponsorships = $sponsor->getSponsorships()->filter(function (Sponsorship $sponsorship) use ($status, $counterpartType, $minAmount, $maxAmount, $otherCounterpart, $hasContract) {
                return (!$status || strtolower($sponsorship->getStatus()->value) === strtolower($status)) &&
     ...
     *
     * @param SponsorCreateDto $sponsorDto The data transfer object containing the sponsor information.
     ...
     */
    public function createSponsor(SponsorCreateDto $sponsorDto, ?UploadedFile $sponsorImage, array $contractFiles): Sponsor
    {
        // create sponsor
        $sponsor = new Sponsor();
     ...
     * @param UploadedFile[] $contractFiles An array of contract files to be associated with the sponsorships.
     */
    public function updatePersonSponsor(Sponsor $sponsor, SponsorDto $sponsorDto, ?UploadedFile $sponsorImage, array $contractFiles): Sponsor
    {
        // update sponsor
        $sponsor->setName($sponsorDto->name)
            ->setDescription($sponsorDto->description)
    ...
    private function updateSponsorships(Sponsor $sponsor, array $sponsorshipsDto, array $contractFiles): void
    {
        ...
        foreach ($sponsorshipsDto as $key => $sponsorshipDto) {
            ...
            $sponsorship->setStatus($sponsorshipDto->status);
